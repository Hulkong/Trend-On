...
// 		var color =["#d4cebf", "#ff8a4a"];
		var colorMap = {
				color : ["#d4cebf", "#ff8a4a"],
				color_float : ["#d3d3d3", "#2ec5c5"],
				color_amt : ["#d3d3d3", "#fe8d75"]
		}
		...
		function setGroupedData(jChartArr, bar_type){
			...
			for(var i = 0 ; i < jChartArr.length ; i++){
				var colorArr = colorMap[ chart_config.colorArrName ]
		...
		function setBarData(jChartArr){
			...
			for(idx; idx< jChartArr.length; idx++){
					...
					chartData.push({
						...
						"color": (chart_config.color != ''? chart_config.color : "#ff8a4a")
					});	
			...
			var yAxis = d3.svg.axis()
							  ...
							  .tickFormat(function(d){
								  var tCnt =  Math.floor(1e-12 + Math.log(d) / Math.LN10);
									if(tCnt == 8 || tCnt == 12){
										return  getIndexNumber(d, 2, true) +chart_config.unit;
									}else{
