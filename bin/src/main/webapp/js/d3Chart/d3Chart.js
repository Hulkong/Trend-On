...
//				                .style("top",  function(){ return y(d.yEnd)+"px";});	
				        ...
				        .on("mousemove", function(d){
				        	div.style("left", function(){
					        		...
					        		if(divId == "#cntBar1"){
//					        			left =  d3.event.pageX - margin.left - margin.right - x0.rangeBand() - x1.rangeBand();
					        			left =  d3.event.pageX - cwidth + margin.right + margin.left + x0.rangeBand() + x1.rangeBand();
					        		}else if(divId == '#cntBar2'){
					        			left += 200;
					        		}
	    ...
	    legend.append("rect")
	      ...
	      .on("mouseover", function(d) {	
	            ...
	            div.html( function(){ 
		            ...
		            .style("top",  function(){
		            	if(divId == "#cntBar1" || divId == "#cntBar2"){
		            		return d3.event.pageY - margin.top + margin.bottom;
		        		}else{
//					        			// 마우스의 위치 - 첫번째 섹션의 높이 - (지도 혹은 그래프의 높이 * 2) - (지도 혹은 그래프의 top, bottom padding 값 ) - 항상 따라다니는 top의 높이 +(이 그래프의 margin bottom)
      ...
      startSlider.constrain(function(event, ui) {
		  ...
		  if(standards.month){//최소 3일 최대 6개월
			  ...
			  if(startTime < endTime){	
				  endSliderDate.setDate(endSliderDate.getDate()+3);
				  endSliderDate.setMonth(endSliderDate.getMonth()- Number(standards.month));
//				  ui.position.left = Math.max(ui.position.left, x(endTime - (1000 *60 * 60 *24 *30*month)));
      ...
      endSlider.constrain(function(event, ui) {
          ...
          if(standards.month){        	  
        	  ...
        	  if(startTime <= endTime){
        		  startSliderDate.setMonth(startSliderDate.getMonth() + Number(standards.month));
        		  startSliderDate.setDate(startSliderDate.getDate()-3);
...
//    	  var x1 = new Date(x0.getTime() + term);//x.invert(parseInt(currentX)+parseInt(sWidth));
    	  var x1 = new Date(dateRound(x0).getTime() + term);//x.invert(parseInt(currentX)+parseInt(sWidth));
